...
The `KataReset` component uses the theming context's `Consumer`, so we can easily override the default theme with a `Theme` component. The default values that can be overridden are as follows:
